...
$sql_semesters = "SELECT DISTINCT semester FROM materials WHERE material_title = 'SYLLABUS' ORDER BY semester ASC";
...
// Fetch syllabus based on the selected semester (if any)
$selected_semester = isset($_GET['semester']) ? intval($_GET['semester']) : null;
$selected_subject = isset($_GET['subject']) ? $_GET['subject'] : null;
$sql_syllabus = "SELECT * FROM materials WHERE material_title = 'SYLLABUS'";
if ($selected_semester) {
    $sql_syllabus .= " AND semester = $selected_semester";
}
if ($selected_subject) {
    $sql_syllabus .= " AND subject = '" . $conn->real_escape_string($selected_subject) . "'";
}
$sql_syllabus .= " ORDER BY semester ASC, subject ASC";
$result_syllabus = $conn->query($sql_syllabus);

// Initialize an array to group syllabus by semester
$syllabus_by_semester = [];
if ($result_syllabus->num_rows > 0) {
    while ($row = $result_syllabus->fetch_assoc()) {
        $syllabus_by_semester[$row['semester']][] = $row;
    }
}
...
<body>
    <div class="container">
        <h1>📋 Available Syllabus</h1>

        <!-- Filter Section -->
        <div class="filter-section">
            <h2><i class="fas fa-filter"></i> Filter Syllabus</h2>
        ...
        <?php if (!empty($syllabus_by_semester)) { ?>
            <?php foreach ($syllabus_by_semester as $semester => $syllabus_list) { ?>
                <div class="semester-section">
                    <div class="semester-header" onclick="toggleVisibility('syllabus-sem<?php echo $semester; ?>')">
                        <span>Semester <?php echo $semester; ?></span>
                        <i class="fas fa-chevron-down"></i>
                    </div>
                    <div id="syllabus-sem<?php echo $semester; ?>" class="notes-list">
                        <table>
                            <thead>
                                <tr>
                                    <th>Subject</th>
                            ...
                            <tbody>
                                <?php foreach ($syllabus_list as $syllabus) { ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($syllabus['description']); ?></td>
                                        <td class="file-cell">
                                            <?php 
                                            $hasFile = !empty($syllabus['material_file']);
                                            $hasLink = !empty($syllabus['external_link']);
                                            ?>

                                            <?php if ($hasFile): ?>
                                                <?php
                                                $file_path = '../admin/' . $syllabus['material_file'];
                                                ?>
                                                <!-- Download link -->
                                                ...
                                                <a href="view.php?id=<?php echo (int)$syllabus['id']; ?>" target="_blank">
                                            ...
                                            <?php if ($hasLink): ?>
                                                ...
                                                <a href="<?php echo htmlspecialchars($syllabus['external_link']); ?>" target="_blank" rel="noopener noreferrer">
        ...
        <?php } else { ?>
            <div class="no-results">
                ...
                <h3>No syllabus found for the selected filters.</h3>
