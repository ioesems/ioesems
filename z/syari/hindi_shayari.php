...
<style>
  body { padding: 32px; font-family: 'Noto Sans Devanagari','Segoe UI',sans-serif; background: linear-gradient(120deg,#f7f7f8,#fff7f0); }
  .card { border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.06); }
  .header { display:flex; align-items:center; gap:12px; }
  .category-buttons .btn { margin:3px; border-radius:20px; }
  .found-shayari { display:none; white-space:pre-line; background:#fff; padding:18px; border-radius:10px; border-left:5px solid #6a1b9a; }
  .spinner { border:4px solid #f3f3f3; border-top:4px solid #6a1b9a; border-radius:50%; width:38px; height:38px; animation:spin 1s linear infinite; margin:0 auto 12px; }
  @keyframes spin { 0% { transform: rotate(0); } 100% { transform: rotate(360deg); } }
  .meta { font-style:italic; color:#555; margin-top:8px; }
  ...
  <div class="card p-4">
    <div class="header mb-3">
      <svg width="46" height="46" viewBox="0 0 24 24" fill="none" style="flex-shrink:0;">
        <path d="M4 6h16M4 12h16M4 18h16" stroke="#6a1b9a" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
      <div>
        <h3 style="margin:0;color:#6a1b9a;">Categorical Shayari Finder</h3>
        <div class="note">Choose a category or pick "Random". The system will keep retrying until it gets a valid quote. Press Stop to cancel.</div>
      </div>
    </div>
    ...
    <div class="mt-3">
      <small class="text-muted">API reference: <code>https://hindi-quotes.vercel.app/random</code> and category subpaths like <code>/love</code>, <code>/motivational</code>, etc.</small>
    </div>
  </div>
</div>
